...
class _HtmlEditorWidgetMobileState extends State<HtmlEditorWidget> {
  ...
  /// Prevents focus-time keyboard/scroll adjustment from running repeatedly
  /// while the user is typing in the same editor focus session.
  bool didAdjustForCurrentFocus = false;
  ...
  void resetHeight() async {
    if (mounted) {
      ...
      await widget.controller.editorController!.evaluateJavascript(
          source:
              "console.log('[JS][mobile-height] reset note-editable outerHeight=${widget.otherOptions.height - (toolbarKey.currentContext?.size?.height ?? 0)}');\$('div.note-editable').outerHeight(${widget.otherOptions.height - (toolbarKey.currentContext?.size?.height ?? 0)});");
    }
  }
              ...
              Expanded(
                child: InAppWebView(
                  ...
                  onConsoleMessage: (controller, message) {
                    // debugPrint('[JS][mobile-console] ${message.message}');
                  },
                  onWindowFocus: (controller) async {
                    if (didAdjustForCurrentFocus) {
                      // debugPrint('[Flutter][mobile-focus] skip repeated focus adjustment');
                      return;
                    }
                    didAdjustForCurrentFocus = true;
                  ...
                  onLoadStop: (InAppWebViewController controller, Uri? uri) async {
                    ...
                    if (url.contains(filePath)) {
                      ...
                      await controller.evaluateJavascript(source: "console.log('[JS][mobile-init] selection listener installed');");
                      ...
                      if (widget.htmlEditorOptions.adjustHeightForKeyboard) {
                        ...
                        keyboardVisibilityController.onChange.listen((bool visible) {
                          if (!visible && mounted) {
                            didAdjustForCurrentFocus = false;
